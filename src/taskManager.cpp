...
bool TaskManager::deleteTask(const std::string &id) {
  auto iterator =
      std::remove_if(tasks.begin(), tasks.end(),
                     [&id](const std::unique_ptr<Task>
                               &task) { // => lambda function finding task by id
...
bool TaskManager::updateTask(const std::string &id,
...
std::vector<Task *>
TaskManager::searchByTitle(const std::string &keyword) const {
  ...
  std::transform(lowerKeyword.begin(), lowerKeyword.end(), lowerKeyword.begin(),
                 ::tolower); // => converting all to lowercase
  for (const auto &task : tasks) {
  ...
  std::sort(sortedTasks.begin(), sortedTasks.end(),
            [](const Task *a, const Task *b) {
              return a->getDeadline() <
                     b->getDeadline(); // => lambda function to compare
                                       // deadlines for sorting
            });
  return sortedTasks;
}
...
// UTILITIES
size_t TaskManager::getTaskCount() const { return tasks.size(); }
